...
void solve(int & drivers, int & races, int & prints) {
    ...
    Clasament clasament[1001];
      // asa voi stoca clasament de la ultimul print
    ...
    for (int i = 1; i <= races + prints; ++i) {
        ...
        if (!(input == "print")) {
            ...
            // in laps o sa am masinile ordonate dupa
            // cum trebuie sa primeasca punctele
            points_received = (drivers - nulled) / 2;
            for (int i = 1; i <= drivers - nulled; ++i) {
                if ((drivers - nulled) % 2 == 0 && points_received == 0)
                    points_received--;
              ...
              // scriu noul laps_ant
            Clasament aux[1002];
            // copiez totul intr un clasament aditional
            for (int i = 1; i <= drivers; ++i) {
                aux[i] = clasament[i];
            }
            for (int i = 1; i <= drivers; ++i)
            // ordonez clasamentul care sa pregateste sa fie afisat
            {
            ...
            if (first_print == 1) {
                for (int i = 1; i <= drivers; ++i) {
                    g << aux[i].index_masina << " ";
                    g << aux[i].car_points << " 0" << std::endl;
                ...
                for (int i = 1; i <= drivers; ++i) {
                    g << aux[i].index_masina << " " << aux[i].car_points;
                    g << " " << aux[i].clasament_vechi - i << std::endl;
                    clasament[aux[i].index_masina].clasament_vechi = i;
                }
            }
            g << std::endl;
